  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        particle.style.animation = `floating ${duration}s ease-in-out ${delay}s infinite, pulse ${
          Math.random() * 4 + 2
  ...
  return (
    <div className="relative overflow-hidden bg-white pt-20 min-h-screen flex items-center">
      ...
      <div className="absolute inset-0 z-0">
        <div
          className={`absolute top-0 right-0 w-1/2 h-1/2 bg-mindmesh-soft-purple rounded-bl-[100px] opacity-30 transition-all duration-1000 ease-out ${
            isLoaded ? "scale-100 rotate-0" : "scale-90 rotate-12"
          }`}
        ></div>
        <div
          className={`absolute bottom-0 left-0 w-1/3 h-1/3 bg-mindmesh-soft-blue rounded-tr-[70px] opacity-30 transition-all duration-1000 delay-300 ease-out ${
            isLoaded ? "scale-100 translate-x-0" : "scale-90 -translate-x-20"
          }`}
        ></div>
      </div>
        ...
        <div className="flex flex-col-reverse lg:flex-row items-center justify-between gap-12">
          <div className="w-full lg:w-1/2 space-y-8 text-center lg:text-left">
            <div className="space-y-4">
              <span
                className={`inline-block px-4 py-1 rounded-full bg-mindmesh-soft-purple text-mindmesh-purple text-sm font-medium transform transition-all duration-1000 ease-out ${
                  isLoaded
              ...
              <h1
                className={`text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-gray-900 font-poppins transition-all duration-1000 delay-200 ease-out ${
                  isLoaded
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-10"
                }`}
              ...
              <p
                className={`text-xl text-gray-600 max-w-xl mx-auto lg:mx-0 transition-all duration-1000 delay-400 ease-out ${
                  isLoaded
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-10"
                }`}
              >
                Upload PDFs or select topics, customize difficulty, and let AI
                generate professional quizzes you can share with anyone.
              </p>
            </div>

            <div
              className={`flex flex-col sm:flex-row justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-4 transition-all duration-1000 delay-600 ease-out ${
                isLoaded
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-10"
              }`}
            >
              ...
              <Button
                variant="outline"
                className="border-mindmesh-purple text-mindmesh-purple hover:bg-mindmesh-soft-purple px-8 py-6 text-lg rounded-lg relative overflow-hidden group"
              >
            ...
            <div
              className={`flex items-center justify-center lg:justify-start space-x-8 pt-4 transition-all duration-1000 delay-800 ease-out ${
                isLoaded
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-10"
              }`}
          ...
          <div
            className={`w-full lg:w-1/2 relative transition-all duration-1000 delay-500 ease-out ${
              isLoaded
